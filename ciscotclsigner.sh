...
usage(){
    print_version
    echo 
    echo " tclsigner.sh -m <Mode> [-f <File>] [-c <Cert>] [-k <Private Key>] [[-d] [-h]]  "
...
if [ "$?" -eq 0 ]; then
    ...
    _log "$TRACE DEBUG:$CLEAR Processing Cisco header, converting binary file $FILE.pk7 to hex $FILE.hex"
...
# if selected mode is "sign" we want path to certificate and private key
    ...
    if [ -z "$FILE" ] || [ -z "$CERT" ] || [ -z "$PRIVATE_KEY" ]; then
        echo "$ERROR ERROR:$CLEAR Specify a file to sign, a certificate and a private key\n"
        usage
        exit 1
...
# if selected mode is "verify" we want the path to certificate and file to verify
    ...
    if [ -z "$FILE" ] || [ -z "$CERT" ]; then
        echo "$ERROR ERROR:$CLEAR Specify a file to verify and the certificate\n"
        usage 
        exit 1
...
# if selected mode is "genkey" we generate keys and Ca cert        
    ...
    if [ -z "$CERT"  ] || [ -z "$PRIVATE_KEY" ]; then
        echo "$ERROR ERROR:$CLEAR Specify certificate and private key name\n"
        usage
        exit 1
    else
        gen_cert
        if [ "$?" -eq 0 ]; then
            echo "$INFO INFO:$CLEAR Generated private key '$PRIVATE_KEY' and certificate '$CERT'\n"
            exit 0
        else
